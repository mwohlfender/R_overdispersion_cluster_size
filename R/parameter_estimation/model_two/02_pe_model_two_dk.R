...
# Project: R and k
# Estimate parameters from cluster data from Denmark (model 2) (constant value for testing probability)
...
# run model
results_parameter_estimation <- estRodis_estimate_parameters_two(clusters_size = data_cluster_sizes_dk_month |> pull("size"),
                                                                 clusters_freq = data_cluster_sizes_dk_month |> pull("frequency"),
                                                                 testing_proba = testing_probas_model_two_dk_2021_months[month_0],
...
saveRDS(results_parameter_estimation, file = paste0("results/parameter_estimates_model_two_denmark_", month_0, ".rds"))

# save session info
writeLines(capture.output(sessionInfo()), con = paste0("sessionInfo_model_two_denmark_", month_0, ".txt"))
