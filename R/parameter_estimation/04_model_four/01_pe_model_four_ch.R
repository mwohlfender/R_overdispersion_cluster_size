...
# Project: R and k
# Estimate parameters from cluster data from Switzerland (model 4)
# (prior distributions for R and k and constant values for mutation probability and testing probability)
...
# run model
results_parameter_estimation <- estRodis_estimate_parameters_four(clusters_size = data_cluster_sizes_ch_month %>% pull("size"),
                                                                  clusters_freq = data_cluster_sizes_ch_month %>% pull("frequency"),
                                                                  prior_r = c(10, 10),
                                                                  prior_k = c(5, 10),
                                                                  mutation_proba = 0.2811,
                                                                  testing_proba = testing_probas_model_four_ch_2021_months[month_0],
                                                                  ...
                                                                  sample_file = paste0("results/parameter_estimates_model_four_switzerland_", month_0, "_sample"),
                                                                  diagnostic_file = paste0("results/parameter_estimates_model_four_switzerland_", month_0, "_dia"))

# save results of parameter estimation
saveRDS(results_parameter_estimation, file = paste0("results/parameter_estimates_model_four_switzerland_", month_0, ".rds"))

# save session info
writeLines(capture.output(sessionInfo()), con = paste0("sessionInfo_model_four_switzerland_", month_0, ".txt"))
