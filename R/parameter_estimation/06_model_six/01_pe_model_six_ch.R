...
# Project: R and k
# Estimate parameters from cluster data from Switzerland (model 6)
# (prior distributions for R, k and mutation probability and constant value for testing probability)
...
# run model
results_parameter_estimation <- estRodis_estimate_parameters_six(clusters_size = data_cluster_sizes_ch_month %>% pull("size"),
                                                                 clusters_freq = data_cluster_sizes_ch_month %>% pull("frequency"),
                                                                 prior_r = c(10, 10),
                                                                 prior_k = c(5, 10),
                                                                 prior_mutation = c(27, 68),
                                                                 testing_proba = testing_probas_model_six_ch_2021_months[month_0],
                                                                 ...
                                                                 sample_file = paste0("results/parameter_estimates_model_six_switzerland_", month_0, "_sample"),
                                                                 diagnostic_file = paste0("results/parameter_estimates_model_six_switzerland_", month_0, "_dia"))

# save results of parameter estimation
saveRDS(results_parameter_estimation, file = paste0("results/parameter_estimates_model_six_switzerland_", month_0, ".rds"))

# save session info
writeLines(capture.output(sessionInfo()), con = paste0("sessionInfo_model_six_switzerland_", month_0, ".txt"))
