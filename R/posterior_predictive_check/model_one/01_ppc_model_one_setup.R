...
paths_results_model_one <- c(path_results_model_one_ch, path_results_model_one_dk, path_results_model_one_de)
...
data_parameters <- data.frame(matrix(data=0, nrow=n_countries*12*n_parameter_combinations*n_repetitions, ncol=12))
names(data_parameters) <- c("country", "month", "index_param_comb", "index_repetition", "n_clusters", "max_cluster_size", "R", "k", "yearly_mutation_rate", "mean_generation_interval", "testing_proba", "sequencing_proba")
...
for (cc in 1:n_countries) {
  ...
  for (mm in 1:12) {
    ...
    data_results_details_cc_mm <- readRDS(file = paste0(paths_results_model_one[cc], as.character(mm), ".rds"))
    ...
    parameters <- data.frame(n_clusters = rep(data_cases_sequences_clusters_cc$Noc[mm], n_parameter_combinations),
                             ...
                             yearly_mutation_rate = sample(data_results_details_cc_mm_samples[,,3][1:n_samples], n_parameter_combinations),
    ...
    data_parameters[(index_start+1):(index_start+n_parameter_combinations*n_repetitions),
                    c("n_clusters", "R", "k", "yearly_mutation_rate", "testing_proba", "sequencing_proba")] <- parameters
    
  }
  
}

write_csv(data_parameters, file = path_data_post_pred_model_one_parameters_grid)

write.table(1:nrow(data_parameters), file = path_data_post_pred_model_one_index_parameters, sep = "", append=F, row.names=F, col.names=F)
